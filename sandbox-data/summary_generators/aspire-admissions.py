"""Daily admission facts built in PostgreSQL with one additive aggregate.

Run: python manage.py admissions_summary --regenerate
Optional: --from YYYY-MM-DD --through YYYY-MM-DD, or --date YYYY-MM-DD.

One row per (date, facility, payer, referral source) that had admissions. Parent
location totals are report GROUP BY results, never stored copies, so no scope can
be double counted with its own children and no empty scope-day is materialised.
Non-additive metrics (distinct referring hospitals) stay derivable because the
source name is kept at the grain. Selected dates publish atomically.
"""
...
FACTS_SQL = """
INSERT INTO __STAGE__ (summary_date, facility_id, payer_id, source_type, source_name,
    admissions, readmissions, readmissions_30_day, medicaid_pending_admissions)
SELECT a.admission_date, s.facility_id, a.payer_id, a.source_type, a.source_name,
       COUNT(*),
       COUNT(*) FILTER (WHERE a.is_readmission),
       COUNT(*) FILTER (WHERE a.is_30_day_readmission),
       COUNT(ma.stay_id)
FROM admission_logs a
JOIN __DATES__ d ON d.summary_date = a.admission_date
JOIN res_stays s ON s.stay_id = a.stay_id
LEFT JOIN medicaid_applications ma ON ma.stay_id = a.stay_id
GROUP BY a.admission_date, s.facility_id, a.payer_id, a.source_type, a.source_name
"""


class AdmissionsSummaryGenerator(BaseGenerator):
    name = 'admissions_summary'
    table = schema.daily_admission_facts
    depends_on = ('admission_logs',)
    ...
    def prepare_sources(self, connection):
        if self.progress:
            self.progress.set_phase('Check admission logs')
        logs, stays = self.admission_logs, self.res_stays
        ...
        if missing is not None:
            raise ValueError('Some saved stays have no admission log. Refresh admission_logs before summarizing.')
        if self.progress:
            self.progress.set_phase('Read summary date range')
        first, last = connection.execute(select(
            func.min(logs.c.admission_date), func.max(logs.c.admission_date))).one()
        ...
        if last and last > self._end:
            raise ValueError('Future admission logs exist. Use seed --reset-history to replace legacy ADT history.')

    def generate(self):
        raise ValueError('Admission facts are built directly with INSERT ... SELECT.')
    ...
    def write_dates(self, connection, days, *, replace_all=False):
        ...
        connection.exec_driver_sql('''
            CREATE TEMP TABLE _admission_fact_dates ON COMMIT DROP AS
            SELECT unnest(%(days)s::date[]) AS summary_date
        ''', dict(days=days))
        connection.exec_driver_sql('CREATE UNIQUE INDEX ON _admission_fact_dates (summary_date)')
        connection.exec_driver_sql('ANALYZE _admission_fact_dates')

        # One pass over the selected admissions. No per-scope or per-day query.
        if self.progress:
            self.progress.set_phase('Aggregate admissions', details=f'{len(days):,} dates; one source scan')
        stage = self.create_stage(connection, self.table)
        statement = FACTS_SQL.replace('__STAGE__', sql.Identifier(stage).as_string(driver))
        statement = statement.replace('__DATES__', '_admission_fact_dates')
        ...
        # Replace the selected dates in the same transaction that publishes them.
        ...
        if replace_all:
            ...
            removed = connection.exec_driver_sql('''
                DELETE FROM daily_admission_facts f USING _admission_fact_dates d
        ...
        for temporary in (stage, '_admission_fact_dates'):
...
GENERATORS = (AdmissionsSummaryGenerator,)


class DailyAdmissionsSummary(DailyGenerator):
    name = 'admissions_summary'
    table = schema.daily_admission_facts
    owned_tables = (table,)
    depends_on = ('adt',)
    ...
    def prepare_daily(self, connection):
        self._summary = AdmissionsSummaryGenerator(self.database_url)
    ...
    def run_dates(self, connection, days):
        ...
        if self.SIMULATION_START in requested:
            # Opening admissions retain their late-2022 dates and belong to the
            # first simulation checkpoint.
...
DAILY_GENERATORS = (DailyAdmissionsSummary,)
