...
- A full-screen modal covers the report header, so a table in one repeats the report's own filters (Payers, Source type and so on) through the `Table`/`AllFacilitiesModal` `filters` slot. Use the same filter component the header uses, so both change the same selection.
